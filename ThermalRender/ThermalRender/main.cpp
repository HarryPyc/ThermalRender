...
int main() {
	...
	int Samples = 0;
	float* d_data, *h_data;
	size_t data_size =  WINDOW_WIDTH * WINDOW_HEIGHT;
	...
	h_data = new float[WINDOW_WIDTH * WINDOW_HEIGHT];
	gpuErrchk(cudaMemcpy(&h_data[0], &d_data[0], data_size * sizeof(float), cudaMemcpyDeviceToHost));

	std::string name = "output/Human&Car_" + std::to_string(TargetSample) + "_depth.txt";
	std::ofstream outFile(name);

	for (int y = 0; y < WINDOW_HEIGHT; y++) {
		for (int x = 0; x < WINDOW_WIDTH; x++) {
			float* ptr = h_data + y * WINDOW_WIDTH + x;
			outFile << *ptr;
			outFile << " ";
		}
		outFile << "\n";
